...
### 리프레시 토큰 관리: RDB(관계형 데이터베이스) vs Redis

리프레시 토큰이 재사용되었는지를 확인하기 위해 최신 리프레시 토큰을 관리해야 했다. 우선 기존에 사용 중인 RDB인 PostgreSQL의 `meeting_user` 테이블에 `refresh_token` 컬럼을 추가하는 방안을 검토했다. 이 방법은 새로운 인프라 구축 없이 기존 시스템을 활용할 수 있다는 장점이 있었다.

**RDB 사용시 문제점**

하지만 검토 과정에서 RDB를 사용하는 것에는 다음과 같은 문제가 있었다.
...
3. **단기 데이터 관리의 비효율성:** RDB는 장기 데이터 관리에 최적화되어 있다. 반면 리프레시 토큰은 짧은 생명주기를 가진 데이터로, 이를 효율적으로 관리하기 위해서는 추가적인 만료 데이터 정리 작업이 필요했다.
...
- [레디스와 RDB의 차이점 및 선택 기준](https://f-lab.kr/insight/redis-vs-rdb)
