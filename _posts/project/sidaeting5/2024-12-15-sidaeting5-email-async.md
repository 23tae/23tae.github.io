...
비동기 처리 도입 후 실제 성능 개선 수치를 확인하기 위해 로컬 환경에서 k6를 활용하여 부하 테스트를 진행하였다. 네트워크 지연 변수를 통제한 상태에서, 가입자가 몰리는 피크 타임을 가정하여 가상 유저(VU) 5명이 1초 대기 시간을 두고 30초 동안 지속적으로 이메일 발송 API를 호출하는 상황(최대 5 TPS)을 모사하였다.

| 측정 지표 | 개선 전 (Before) | 개선 후 (After) | 개선 효과 |
| :--- | :--- | :--- | :--- |
| **평균 응답 속도 (avg)** | 2.07s | **12.15ms** | **99.4% 단축** |
| **최대 응답 속도 (max)** | 2.48s | **32.24ms** | **98.7% 단축** |
| **초당 처리량 (TPS)** | 1.62 req/s | **4.92 req/s** | **3배(300%) 향상** |

|![기존](/assets/img/project/sidaeting5/02-email-verification/k6-email-before-async.png)|![개선 후](/assets/img/project/sidaeting5/02-email-verification/k6-email-after-async.png)|
|:---:|:---:|
|_개선 전 k6 테스트 결과_|_개선 후 k6 테스트 결과_|

- **클라이언트 대기 시간 대폭 감소**: 기존 동기 방식에서는 외부 API(SES) 응답을 기다리느라 평균 2.07초가 소요되었으나, 비동기 처리 도입 후 12.15ms 만에 응답을 반환하여 체감 속도를 크게 개선했다.
- **서버 처리량(TPS) 향상**: HTTP 연결이 즉시 종료되고 메인 스레드가 반환됨에 따라, 초당 처리량(TPS)이 1.62에서 4.92로 3배가량 향상되었다.

## 결론

1. **성능 개선:** 이메일 전송 비동기 처리를 통해 API 응답 시간을 99% 이상 단축시키고 서버 처리량을 3배 이상 향상시켰다.
