...
이번 프로젝트는 기존 어카운트 서버를 사용하지 않았기 때문에 별도의 유저 인증 기능 개발이 필요했다. 이에 **JWT 기반의 인증 기능**을 구현하였다.
...
추가적으로 프로젝트 초기에는 로그인 기능(아이디-비밀번호 방식, 소셜 로그인 방식)도 논의했으나, 단기 서비스 특성상 비효율적이라는 의견이 있어 이메일 인증을 통해 재학생임을 확인하고, **토큰을 길게 발급**하여 추가 인증을 요구하지 않는 방식을 사용하기로 결정했다.
