...
캐싱 전략 도입 후의 성능 개선을 검증하기 위해 로컬 환경에서 k6를 활용하여 부하 테스트를 진행하였다. 네트워크 지연 변수를 통제한 상태에서, 매칭 결과 발표 시점의 대규모 트래픽 스파이크를 모사하기 위해 가상 유저(VU) 50명이 1초 대기 시간을 두고 30초 동안 지속적으로 API를 호출하는 상황(최대 50 TPS)을 가정하였다.

| 측정 지표 | 개선 전 (Before) | 개선 후 (After) | 개선 효과 |
| :--- | :--- | :--- | :--- |
| **평균 응답 속도 (avg)** | 68.63ms | **18.88ms** | **72.5% 단축** |
| **상위 95% 속도 (p95)** | 113.35ms | **60.89ms** | **46.2% 단축** |
| **최대 응답 속도 (max)** | 668.91ms | **153.69ms** | **77.0% 단축** |

|![기존](/assets/img/project/sidaeting5/04-matching-api/k6-matching-api-before-caching.png)|![개선 후](/assets/img/project/sidaeting5/04-matching-api/k6-matching-api-after-caching.png)|
|:---:|:---:|
|_개선 전 k6 테스트 결과_|_개선 후 k6 테스트 결과_|

- **DB 부하 원천 차단 및 평균 속도 단축**: 여러 테이블을 조인해야 했던 쿼리를 Redis 캐시로 대체하여 평균 응답 속도를 68.63ms에서 18.88ms로 약 72.5% 단축했다.
- **지연 시간 억제**: 대규모 동시 접속 시 발생하던 최대 응답 지연(Max)이 668ms에서 153ms로 77% 이상 억제되어, 트래픽이 몰려도 안정적인 속도를 보장한다.
- **캐시 워밍의 효과**: 발표 전 데이터를 미리 적재해둔 덕분에 첫 요청부터 캐시 미스 없이 빠른 응답을 반환할 수 있었다.
