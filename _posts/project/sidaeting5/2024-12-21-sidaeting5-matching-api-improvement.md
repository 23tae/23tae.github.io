---
title: "매칭 API 개선기: 성능 최적화와 트래픽 대응 전략"
...
이번 시즌은 매칭 신청 기간 동안 1:1 소개팅과 3:3 미팅에 대해 참가 신청을 받은 뒤, 매칭 알고리즘으로 매칭한 결과를 주말동안 공개하는 방식으로 운영하였다. 특히 매칭 결과 조회는 발표 당일 대규모 트래픽이 몰릴 것으로 예상되었고, 이를 효율적으로 처리하기 위해 API를 지속적으로 개선해야 했다.

![match-result-page](/assets/img/project/sidaeting5/04-matching-api/match-result-page.png)

매칭 결과 조회는 다음과 같은 단계를 포함한다:

1. **참여 내역 조회**: 사용자가 참여한 매칭 신청 정보(1:1 또는 3:3)를 조회.
2. **매칭 성공 여부 조회**: 매칭 성공 또는 실패 여부를 확인.
3. **상대 정보 확인**: 매칭된 상대가 보낸 메시지(편지)를 확인.
...
이러한 테이블 간의 관계가 얽혀 있어 쿼리를 작성하거나 데이터를 추출하는 과정에서 어려움이 있었다.

**대규모 트래픽에 대한 대비**

매칭 결과가 발표되는 시점에는 수백명에서 천명 사이의 신청자가 한꺼번에 매칭 결과를 조회할 것으로 예상되었다. 하지만 데이터베이스 쿼리를 비효율적으로 작성하면 서버 응답 시간이 느려지거나, 심각한 경우 서버가 다운될 수 있었다. 따라서 데이터베이스 최적화와 캐싱 전략 도입이 중요한 과제로 떠올랐다.
...
초기에서는 아래와 같이 서비스 내부에서 `@Cacheable`을 적용한 메소드를 다른 메소드가 호출하는 구조로 설계하였다. 하지만 실제 실행시 캐싱이 제대로 동작하지 않는 문제가 발생했다. 이는 `@Cacheable`의 동작 원리에 대해 충분히 이해하지 못한 채로 사용했기 때문이었다.
...
추가적으로 기존 Pull Request에 대해 대규모 트래픽 발생시 문제가 예상된다는 의견을 받기도 했다. 따라서 API를 재설계하기로 결정했다.
...
이번 매칭 API 개선 작업은 복잡한 테이블 구조를 단순화하고, 대규모 트래픽을 효과적으로 처리할 수 있는 설계를 고민한 좋은 경험이었다. 특히, 캐싱을 통해 성능을 최적화하는 과정에서 캐시의 역할과 동작 방식을 명확히 이해할 수 있었다.
