...
3. **나이(age)**: -5/-3/동갑/+3/+5 중 최소, 최대를 선택하여 범위 지정
4. **키(height)**: 150~190 범위에서 5cm 단위로 범위 지정
5. **흡연(smoking)**: 연초/전자담배/비흡연 중에서 중복 선택
6. **MBTI(mbti)**: I/E, N/S, T/F, P/J 중에서 중복 선택
7. **기피 대상**: 기피학과(avoidanceDepartment), 기피학번(avoidanceNumber) 입력
...
![image.png](/assets/img/project/sidaeting5/06-matching-algorithm/core-preference.png){: width="60%" style="display: block; margin: auto; "}
_핵심 선호 항목 선택 페이지_
...
하지만 이번 매칭 알고리즘 개발은 몇 가지 개선의 여지도 존재한다. 먼저 **[매칭 성공률의 편중 문제](../sidaeting5-match-result-analysis/)**가 있다. **알고리즘의 효율성** 측면에서도 개선이 필요하다.

다음 시즌에는 이러한 문제점을 해결하기 위해 아래와 같은 개선 방향을 고려하면 좋을 것 같다.
...
이번 시즌 신청자들의 정보를 분석한 결과는 다음과 같다. 참가자 학번은 예상대로 1학년인 24학번이 가장 많았으나, 그 다음으로는 각각 21학번과 20학번이 많이 신청한 것으로 나타났다. 흥미로웠던 점은 남녀 모두 선호 MBTI로 ENFJ를 가장 많이 선택했다는 사실이다. 핵심 항목으로는 남성의 경우 얼굴상(또렷/중간/순한)을, 여성은 키를 가장 많이 선택했다. 마지막으로 기피 학과로 자신의 학과를 선택한 사람이 무려 절반에 달한다는 사실도 알 수 있다.

```
<전체>
남녀성비: 307
주요 신청 학번: 24학번, 21학번, 20학번
주요 신청 학과: 경영학부, 전자전기컴퓨터공학부, 경제학부
본인 학과 기피 비율: 50.3%
...
선호 쌍꺼풀: 유쌍
선호 MBTI: ENFJ
핵심 선호 항목: 얼굴상(33%), 흡연 여부(21%), 나이(19%)
본인 학과 기피 비율: 47.8%
