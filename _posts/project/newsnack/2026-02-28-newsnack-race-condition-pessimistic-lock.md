---
title: "AI 파이프라인 동시성 문제 해결: 비관적 락을 활용한 상태 제어"
...
뉴스낵의 [콘텐츠 생성 파이프라인](../newsnack-airflow-pipeline/)이 안정화되면서 운영 환경은 점차 다양해졌다. 내부적으로 자동 스케줄러(Airflow)가 주기적으로 뉴스툰 생성을 스케줄링하는 동시에, 운영자가 임의로 AI 서버(FastAPI)의 API를 호출하여 병렬로 요청이 처리될 수 있는 환경을 마주했다.
...
파이프라인 설계 당시의 핵심 목표는 멱등성이었다. 한 이슈에 대해 여러 번 생성 요청이 들어와도 뉴스툰은 단 1개만 만들어져야 했다.
...
이 문제를 해결하는 방법은 **락이 걸린(=처리 중인) 행을 '바로 무시'**하는 것이다.
