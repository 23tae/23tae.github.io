...
뉴스낵의 백엔드는 Spring Boot API 서버, FastAPI AI 엔진, Airflow 파이프라인이라는 성격이 전혀 다른 세 시스템이 하나의 PostgreSQL 데이터베이스를 공유하는 구조다. 이 복합적인 시스템을 프로덕션 환경에 올리기 위해서는 단순히 서버를 켜는 것을 넘어, **보안·비용·재현성** 세 가지를 동시에 만족하는 인프라 아키텍처를 설계해야 했다.

이 글에서는 초기 VPC 설계부터 Nginx 컨테이너화를 통한 IaC(Infrastructure as Code) 체계 확립까지, 뉴스낵의 AWS 인프라가 완성되어 가는 과정을 정리한다.
...
프라이빗 서브넷의 RDS를 지키는 핵심 방화벽이다. 인바운드 허용 규칙의 소스를 막연한 IP 대역이 아닌, **`newsnack-common-sg` 보안 그룹 ID 자체**로 설정했다.

이 설계의 의미는 명확하다. 외부 인터넷은 이미 서브넷 차원에서 막혀 있고, 내부망이라 할지라도 **퍼블릭 방화벽 그룹의 허가를 받은 앱 서버에서 흘러온 트래픽만** DB 진입을 허용한다. 허가받지 않은 내부 접근조차 원천 차단되는 이중 저지선이 완성된 것이다.

## 3. S3 퍼블릭 서빙: Bucket Policy와 CORS

AI가 생성한 이미지(웹툰, 카드뉴스 썸네일)는 S3에 올라간 뒤 프론트엔드에서 `<img src="...">` 태그로 직접 렌더링된다. 이를 가능하게 하기 위한 설정이 두 가지 필요하다.
...
뉴스 서비스의 기사 이미지는 그 URL만 알아도 누구나 볼 수 있어도 무방한 공개 콘텐츠이므로, Pre-signed URL이나 CloudFront 같은 추가 레이어 없이 버킷 정책으로 단순하게 해결했다.
...
**API 서버의 포트 바인딩 이유**  
API 컨테이너의 경우, CI/CD 배포 스크립트에서 자동화된 헬스체크(`curl http://localhost:8080`)를 매우 간편하게 수행하기 위해 도커 런타임에 호스트 포트를 바인딩(`-p 8080:8080`)해두었다. 호스트 포트가 열려있지만 앞서 구축한 **AWS 보안 그룹(newsnack-common-sg)**에서 8080 인바운드 트래픽을 원천 차단하고 있으므로 외부 공격 위협은 없다. 
이를 통해 AI/Airflow와 같은 민감한 컴포넌트는 호스트 포트를 영구 폐쇄하여 완벽히 은닉하고, API 서버는 클라우드 계층의 방화벽(AWS SG)을 매개 삼아 헬스체크 편의성을 살려내는, **클라우드와 도커를 관통하는 유연한 아키텍처**를 완성할 수 있었다.
