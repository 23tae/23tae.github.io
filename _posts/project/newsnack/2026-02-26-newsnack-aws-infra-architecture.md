...
우선 AWS의 'VPC 등' 생성 마법사를 활용해 2-AZ 기반의 서브넷 분리 구조를 구성했다.
...
![nat_gateway_pricing](/assets/img/project/newsnack/aws-infra-architecture/nat_gateway_pricing.png)

일반적인 3-Tier 아키텍처(Web - WAS - DB)에서는 보안을 위해 WAS를 프라이빗 서브넷에 배치한다. 이 경우 외부 통신을 위해 NAT Gateway가 필수적이지만, 이는 **시간당 고정 요금($0.059, 월 약 $42)**과 트래픽 비용이 동시에 부과되어 초기 프로젝트 단계에서는 큰 부담이다.

우리 팀은 인프라 비용 최소화를 위해 아키텍처를 전략적으로 설계했다. 외부 통신이 전혀 필요 없는 **데이터베이스(RDS)**만 프라이빗 서브넷에 격리하고, 인터넷 통신이 필수적인 앱 컨테이너들(메인 API 서버, AI 서버, Airflow)은 **퍼블릭 서브넷**에 배치했다. 이를 통해 인터넷 게이트웨이(IGW)를 직접 활용함으로써 NAT Gateway 고정 비용을 원천적으로 제거했다. 퍼블릭 배치에 따른 보안 우려는 다중 **보안 그룹 설계**를 통해 애플리케이션 포트 접근을 엄격히 제어하는 방식으로 보완했다.

또한, VPC 구성 시 **S3 Gateway Endpoint(`newsnack-vpce-s3`)**를 추가로 생성하여 활용했다. 이는 추가 과금이 없는 무료 리소스로, 퍼블릭 서브넷의 EC2가 S3와 통신할 때 일반 인터넷망(IGW)이 아닌 **AWS 내부 백본망**을 거치도록 라우팅한다. 이를 통해 데이터 전송 비용을 절감함과 동시에 네트워크 성능과 보안성을 동시에 확보했다.
...
## 3. S3 퍼블릭 서빙: 정적 리소스 호스팅과 CORS 전략

AI가 생성한 이미지(뉴스툰)는 기사 URL만 확보하면 누구나 열람할 수 있는 퍼블릭 콘텐츠다. 이를 원활하게 서빙하면서도 미래의 프론트엔드 확장성을 보장하기 위해, S3를 단순한 이미지 저장소를 넘어 **웹 정적 리소스를 제공하는 퍼블릭 스토리지**로 설계했다.
...
뉴스툰은 불특정 다수가 열람할 수 있는 공개 콘텐츠다. 따라서 임시 권한을 발급하는 **Pre-signed URL**이나 추가 인프라 비용과 복잡도를 유발하는 **CloudFront**를 의도적으로 배제했다. 대신 S3 버킷 레벨의 정책(Bucket Policy)만으로 `s3:GetObject` 권한을 개방하여, 가장 직관적이고 비용 효율적인 아키텍처를 선택했다.

**2. CORS 설정: 클라이언트 확장성 확보**

단순히 `<img>` 태그나 Next.js의 `<Image>` 컴포넌트로 이미지를 렌더링하는 현재의 방식에서는 타 도메인이라도 브라우저의 동일 출처 정책(SOP) 제약을 개별적으로 받지 않는다. 클라이언트 렌더링 자체가 교차 출처 에러를 유발하지 않거나 서버 간 통신으로 처리되기 때문이다.

그럼에도 `AllowedMethods: ["GET", "HEAD"]`를 **CORS 정책에 포함**했다. 프론트엔드의 리소스 활용 방식이 다변화될 때 발생할 수 있는 잠재적 차단 이슈를 원천적으로 예방하기 위함이다.
추후 자바스크립트의 `fetch` API를 활용한 직접 다운로드 기능 구현, `<canvas>` 태그를 이용한 픽셀 조작, 혹은 웹 폰트(`.woff`) 파일처럼 브라우저가 엄격하게 CORS를 검사하는 기타 정적 리소스 서빙이 요구될 때, 인프라의 변경 없이 클라이언트가 유연하게 통신할 수 있도록 대비했다.
...
인프라 아키텍처 수준에서의 엄격한 보안 설정은 상위 애플리케이션 레이어의 취약점을 보완하는 필수적인 요소다. 위와 같은 과정을 거치며 단일 인스턴스 위에서 4개의 서비스 컴포넌트가 강력하게 통제되면서 외부 노출은 최소화된 인프라를 완성할 수 있었다.
