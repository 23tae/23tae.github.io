...
뉴스낵은 Airflow 기반의 파이프라인을 통해 매일 다양한 언론사에서 수집된 기사들을 특정 주제인 '이슈' 단위로 묶어 관리한다. AI 엔진은 이렇게 이슈 단위로 수집된 기사들을 토대로 뉴스툰이나 오늘의 뉴스낵으로 만드는 복합 기능을 제공한다. 

시스템 초기에는 단순한 단발성 프롬프트 호출만으로 AI 워크플로우를 관리하려 했으나, 뉴스 성격에 따라 분기 처리가 필요해지고 생성 단계가 복잡해지면서 기존 방식으로는 워크플로우 제어에 명확한 한계를 마주했다.

## 배경 및 의사결정

단순한 LangChain 체인이나 Gemini SDK 다이렉트 호출 방식은 `1:1` 텍스트 변환에는 적합했다. 하지만 뉴스낵 엔진은 크게 두 가지의 다차원적인 플로우를 요구하고 있었다.
...
2. **데이터 통합:** '오늘의 뉴스낵(오디오 브리핑)'을 만들기 위해서는 앞서 수집된 여러 개의 기사 데이터를 하나로 모아 종합 대본 생성 노드에 전달해야 했다.
