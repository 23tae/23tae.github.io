...
| 모델명 | 역할 배치 | 장당 비용 | 특징 및 장단점 |
| --- | --- | --- | --- |
| **Gemini 3.1 Flash Image** | **메인 모델** | $0.045 | 빠르고 저렴하다. 과부하 에러가 현저히 적다. |
| **Gemini 3.0 Pro Image** | **우회 모델** | $0.134 | 느리고 비용이 비싸지만 퀄리티는 우수하다. 트래픽 과부하 빈도가 높다. |

이 결정의 핵심은 **비용 절감과 가용성 방어**다. 매일 수백 장의 이미지를 생성하는 파이프라인에서, 평소에는 장애 확률이 낮고 저렴한 `Flash` 모델을 **메인 모델**로 배치하여 리소스를 아낀다. 반면 기존에 병목을 유발했던 비싼 `Pro` 모델은 **우회 모델**로 전환했다. 
...
Circuit Breaker(서킷 브레이커)는 전기 회로의 차단기에서 이름을 따온 패턴이다. 과전류가 흐를 때 자동으로 회로를 차단하는 것처럼, 서비스에서 지정된 에러가 임계값을 넘으면 해당 경로를 일시적으로 차단하고 우회 경로로 트래픽을 돌린다.
