...
- **GitHub Secrets 등록**  
  `SENTRY_AUTH_TOKEN`과 `SENTRY_DSN` 같은 민감 정보는 코드에 노출되지 않도록 GitHub Repository의 Secrets에 등록했다.
...
필요한 준비를 모두 마쳤다. 이제 `GlobalExceptionHandler`의 마지막 `handleException(e: Exception)` 메서드에서 처리되지 않은 예외가 발생하면 Sentry SDK가 이를 감지한다. 감지된 에러는 Sentry 대시보드에 이슈로 등록되고, 앞서 설정한 규칙에 따라 Slack 채널로 즉시 알림이 전송된다.

![slack sentry notification](/assets/img/project/eodigo/sentry-error-monitoring/slack_sentry_notification.png)
_Slack 채널에 전송된 Sentry 알림_

![sentry_issue.png](/assets/img/project/eodigo/sentry-error-monitoring/sentry_issue.png)
_Sentry 대시보드에 등록된 이슈_
...
평가판 기간이 종료된 후, Sentry 계정이 무료(Developer) 플랜으로 전환되었고 Slack 연동도 비활성화되었다. 만약 이 프로젝트의 개발을 이어가야 한다면 무료 플랜에서 사용 가능한 알림 시스템으로의 전환이 필요하다.
...
- [Spring Boot \| Sentry for Spring Boot](https://docs.sentry.io/platforms/java/guides/spring-boot/)
- [Options \| Sentry for Spring Boot](https://docs.sentry.io/platforms/java/guides/spring-boot/configuration/options/)
- [[Spring] 서버에서 발생한 예외, Sentry Issue 정보 Slack 알림으로 보내기 \| by Hailey \| Medium](https://medium.com/@Hailey24/spring-%EC%84%9C%EB%B2%84%EC%97%90%EC%84%9C-%EB%B0%9C%EC%83%9D%ED%95%9C-%EC%98%88%EC%99%B8-sentry-issue-%EC%A0%95%EB%B3%B4-slack-%EC%95%8C%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EB%B3%B4%EB%82%B4%EA%B8%B0-dbb7e50ee49b)
