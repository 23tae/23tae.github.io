...
최근 IT 연합 동아리 [DND](https://dnd.ac/) 13기에 참여했다. 우리 팀은 디자이너, 안드로이드 개발자, 백엔드 개발자 각각 2명씩, 총 6명으로 구성되었고, 나는 백엔드 개발을 맡았다. 8주간의 활동 기간 동안 하나의 프로덕트를 완성하는 것이 목표였다. 
...
서비스의 주요 기능은 외식 가격 비교와 식료품 물가 탐색, 두 가지로 나뉜다.

1. **외식 가격 비교:** 특정 메뉴(e.g., 삼겹살 1인분)를 기준으로 주변 상점의 가격 정보를 비교하여 제공한다.
2. **식료품 물가 탐색:** 내가 담당한 파트로, 공공데이터(KAMIS)를 활용한다.
    - **가격 랭킹:** 특정 품목의 최신 데이터를 기준으로 지역별 가격 순위를 제공한다.
...
- **인프라 및 배포: `AWS`, `Docker`, `GitHub Actions`**
    - **선택 이유:**
...
프로젝트의 전체 인프라 구조이다. 보안을 최우선으로 고려하여 데이터베이스(RDS)는 Private Subnet 내에 배치했고, 애플리케이션 서버(EC2)를 통해서만 접근 가능하도록 네트워크를 구성했다. 상세한 AWS 인프라 구축 과정은 별도의 글에서 다룰 예정이다.
...
서비스의 핵심인 최신 가격 데이터 수집은 매일 새벽 Spring Batch Job을 통해 수행됐다. 이 파이프라인은 스케줄러에 의해 자동으로 실행되며, KAMIS Open API로부터 데이터를 조회하여 DB에 저장한다. 상세한 파이프라인 설계 및 구현 과정은 이후에 자세히 소개할 예정이다.

## 마치며

4주간의 MVP 개발 기간 동안 핵심 기능을 구현하고 배포까지 완료했다. 이 과정을 통해 클라우드 인프라 설계부터 CI/CD, 데이터 배치 처리까지 백엔드 개발의 전반적인 사이클을 경험할 수 있었다.

시간 제약으로 인해 못한 Flyway를 이용한 DB 마이그레이션 자동화나 Prometheus, Grafana 기반의 모니터링 시스템 구축은 추후 과제로 남겨두었다.
