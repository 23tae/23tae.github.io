...
이번 주에 42서울 라피신 rush01 평가 봉사를 하면서 int형 배열의 길이를 `while(arr != '\0')`로 확인할 경우 `-fsanitize=address -g` 로 옵션을 지정하여 컴파일하면 **buffer-overflow**가 발생할 수 있다는 사실을 알게됐다.
...
이 방식을 사용할 때의 주의할 점은 **배열 부식**이 발생할 수 있다는 것이다. 예를 들어 길이가 5인 배열의 요소의 개수를 구하는 함수를 정의하여 배열을 매개변수로 받은 경우 `sizeof(arr)/sizeof(arr[0])`은 `sizeof(int *)/sizeof(int)`가 되어 2(=8/4)가 나온다.
