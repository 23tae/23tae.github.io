...
- **개념**
    ...
    - 응용프로그램을 구성하는 서비스, 구성 방법 및 응용프로그램 간의 관계를 설명한다.
    - 각 서비스는 동일한 네트워크 내에서 별도의 컨테이너로 정의되어 서로 쉽게 통신할 수 있다.
- **구성**
    - **Services**
        - 이미지, 환경 변수, 볼륨 및 포트를 포함하여 애플리케이션의 각 컨테이너형 서비스를 정의함
        - `volumes`
            - 데이터를 유지시켜준다.
...
![mariadb-logo](/assets/img/42seoul/inception/inception2/Untitled.png){: width="30%" height="30%"}
...
- 특징
    ...
    ![](/assets/img/42seoul/inception/inception2/inception_01.png)
- 사용 방법
...
- 특징
    ...
    ![](/assets/img/42seoul/inception/inception2/inception_02.png)
    - 서버 블록을 명시하는데 사용된다.
...
- make로 이미지 빌드 후 컨테이너 실행
    ![wp](/assets/img/42seoul/inception/inception2/inception_09.png)
- `docker images`, `docker ps`, `docker network ls`, `docker volume ls` 확인
    ![wp](/assets/img/42seoul/inception/inception2/inception_10.png)
- `docker logs -f <Container ID>`로 wordpress 설치가 완료되었는지 확인
    ![wp](/assets/img/42seoul/inception/inception2/inception_11.png)
...
- `https://<intra ID>.42.fr`로 이동
    ...
    - 다음과 같은 경고창이 뜨는데, openSSL을 통해 인증서를 발급받았기 때문이다.
        ![wp](/assets/img/42seoul/inception/inception2/inception_03.png)
        - 인증서 정보는 다음과 같다.
            ![wp](/assets/img/42seoul/inception/inception2/inception_04.png)
...
- 컨테이너 생성 시 Wordpress를 설치했고 wp-cli로 기본적인 설정을 마쳤다면 다음과 같은 화면을 볼 수 있다.
    ![wp](/assets/img/42seoul/inception/inception2/inception_05.png)
- `https://<intra ID>.42.fr/wp-admin`으로 이동한 뒤 admin 계정으로 로그인
    ![wp](/assets/img/42seoul/inception/inception2/inception_06.png)
    - 로그인을 마치면 다음과 같이 관리 페이지가 나온다.
        ![wp](/assets/img/42seoul/inception/inception2/inception_07.png)
    - `Users`탭에서 이전에 등록한 사용자를 확인할 수 있다.
        ![wp](/assets/img/42seoul/inception/inception2/inception_08.png)
...
- contents
    ...
    - [https://stackoverflow.com/questions/39082768/what-does-set-e-and-exec-do-for-docker-entrypoint-scripts](https://stackoverflow.com/questions/39082768/what-does-set-e-and-exec-do-for-docker-entrypoint-scripts)
