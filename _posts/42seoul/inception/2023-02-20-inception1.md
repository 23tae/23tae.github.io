...
### Container 삭제
    ...
    - 컨테이너 목록 확인 (stopped 된 컨테이너도 확인가능)
...
- 관련 플랫폼
    - **도커 스웜(Docker Swarm)**
        ...
        ![swarm-logo](/assets/img/42seoul/inception/inception1/swarm_icon.jpg){: width="30%" height="30%"}
        - 도커 컨테이너 토종(native)의 클러스터링 및 스케줄링 도구
        - 스웜(swarm, 도커  노드들의 묶음)을 만들고 관리할 수 있도록 해준다.
        - 설치와 사용이 간단하다.
    - **쿠버네티스(Kubernetes)**
        
        ![kuber-logo](/assets/img/42seoul/inception/inception1/kubernetes_icon.jpg){: width="30%" height="30%"}
...
- 특징
    ...
    - 기본적으로 **공용 키**와 **대칭 키 암호화**를 통해 클라이언트와 서버 간에 전송되는 데이터를 암호화한다.
    - 발급받은 **인증서**는 보통 **공용 키** 파일과 CA의 **디지털 서명**이 포함하고 있다.

## OpenSSL

- 개념
    - 네트워크를 통한 데이터 통신에 쓰이는 프로토콜인 TLS와 SSL의 오픈 소스 구현판
- `.crt` 확장자의 **공용 키** 파일과 `.key` 확장자의 **개인 키** 파일이 생성된다.
- 인증서 속성
    - `CN`: CommonName
...
![Untitled](/assets/img/42seoul/inception/inception1/Untitled%2011.png)
