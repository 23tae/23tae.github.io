...
![Untitled](/assets/img/42seoul/philosophers/philo4/Untitled.png)
...
## 문제의 해결방법
Bonus 파트는 mandatory와는 다르게 두 철학자 사이에 포크가 놓이는게 아닌, 한 곳에 포크가 모여있는 구조이다. 따라서 교착상태와 기아상태를 mandatory와는 조금 다르게 해결하였다.
...
- [x]  메모리 누수 발생
    ...
    - **발생 원인 1**
        - 우선 아래와 같이 세마포어의 이름을 생성하는 함수 내부에서 사용을 마친 변수를 제대로 해제해주지 않았다.
    ...
    - **해결 방법 1**
        - 세마포어 이름을 생성하는 함수에서는 사용을 마친 변수를 free 해주었다.
    ...
    - **해결 방법 2**
        - 철학자 구조체의 세마포어명이 필요한 경우, 매 순간 이름을 생성하지 않고 맨 처음에만 생성하여 구조체에 저장 후 종료 직전에 free하였다.
        - 해당 부분을 수정하니 더이상의 메모리 누수는 발생하지 않았다.
        
        ![Untitled](/assets/img/42seoul/philosophers/philo4/Untitled%204.png)
        
- [x]  철학자의 사망 메시지가 프로그램의 시작 직후 출력되는 문제
    
    ![Untitled](/assets/img/42seoul/philosophers/philo4/Untitled%205.png)
    
    - 원인 : 철학자 프로세스가 시간 변수에 값을 저장하기 전에 모니터가 값에 접근해서 발생
    - 해결 방법 : 시간 변수를 초기화하는 시점을 모니터 스레드가 생성되기 전으로 변경
- [x]  철학자의 사망 메시지를 출력한 이후에도 다른 철학자 프로세스가 동작하는 문제
