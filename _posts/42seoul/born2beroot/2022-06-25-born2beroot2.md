...
- **/var** : 변경 가능한 상태의 파일을 위한 장소. **variable(변수)**를 의미한다.
...
- Ext3 : 저널링 파일 시스템. 16TB까지 디스크 사용할 수 있다.
    - 저널링 기술 : 시스템의 비정상적인 종료시에도 로그에 저장된 데이터로 백업 및 복구가 가능하다.
- Ext4 : 대부분의 리눅스 배포판이 채택한 저널링 파일 시스템. 최대 1EB의 볼륨을 지원한다. 하위호환성을 가진다.(ext2, ext3을 ext4로 마운트하는것이 가능)
