...
파티션 대신 볼륨 단위로 저장장치를 다뤄서 유연한 용량조절(각 볼륨에 대한)이 가능함
...
서로 다른 시스템에 있는 개체 간에 데이터를 전송하는데 사용하는 통신 규약. 네트워크에 연결된 컴퓨터들끼리 미리 약속한 전송 규칙
...
**공인IP** : 유일한 IP이기 때문에 외부에서 직접 접근이 가능

**사설IP** : 해당 공유기 내에서 부여한 주소이기 때문에 외부에서는 직접 접근이 불가

**포트포워딩 :** 공유기에서 각 기기에 포트를 부여해서 외부로 개방시킨 것
...
보안, 빠른 퍼포먼스, 높은 안정성을 지님. config 설정 방법이 간단해 FTP 서버 관리가 용이함.  
