---
title:  "Born2beroot ④ Mandatory part"
...
- 시스템 계정
    ...
    리눅스는 파일이나 프로세스 생성 시 반드시 소유자를 명시하게 되는데 모든 시스템에 대해 root 권한을 부여할 시에 생길 수 있는 시스템 손상의 문제를 시스템계정을 통해 방지 할 수 있음.
...
중간에 위치한 **per-package modules**를 아래와 같이 변경
...
위 VM 설정에서 호스트 포트를 22(ssh 기본포트)로 지정했다면 ssh로 접속할 때 포트부분을 제외하고 `ssh [username]@[HOST IP]` 만 입력해도 됨
...
- 물리 프로세서 개수(CPU physical)
    ...
    - `cat /proc/cpuinfo | grep "physical id" | sort -u | wc -l`
        - **physical id** : 0은 프로세서가 0개라는 의미가 아님. 0번 id를 갖는 프로세서라는 의미(0, 1, 2, … 으로 증가함)
