...
1. 파일 또는 터미널로부터 **입력**을 받는다.
2. 따옴표 규칙을 지키며 입력을 **단어**와 **연산자**로 쪼갠다. 이러한 토큰들은 **메타문자**로 구분된다.
3. 토큰들을 간단하고 복합된 명령어로 **파싱**한다.
4. 다양한 **shell 확장**을 진행한다.
5. 필요한 **리다이렉션**들을 수행하고 해당 연산자와 피연산자들을 인자 리스트에서 제거한다.
6. 명령을 **실행**한다.
...
![Untitled](/assets/img/42seoul/minishell/mini2/Untitled%203.png)

- 터미널을 실행했을 때 켜지는 사각형의 창
- 사용자로부터 명령어 형식의 **입력을 받아서, 출력을 화면에 표시**하는 역할
- 정보를 처리하는 것이 아님
...
![Untitled](/assets/img/42seoul/minishell/mini2/Untitled%204.png)
...
![Untitled](/assets/img/42seoul/minishell/mini2/Untitled%205.png)
...
[01.리눅스 기초 - 쉘(Shell)과 디렉토리 구조](https://velog.io/@jewon119/01.%EB%A6%AC%EB%88%85%EC%8A%A4-%EA%B8%B0%EC%B4%88-%EC%89%98Shell)