...
- 원리
    1. 통신을 하지 않는동안은 통신회선이 항상 1로 유지된다.
    2. 시작비트인 0이 들어와 회선의 상태가 변하면 데이터를 수신하기 시작한다.
    3. 8비트를 모두 수신하면 정지비트를 수신하고 통신을 종료한다.
- 장점
    - 송수신자 사이에 동기화가 필요 없다.
    - 비용 효율이 높다.
- 단점
...
`sa_flags = SA_SIGINFO` 일 경우 sa_handler 대신 sa_sigaction이 동작한다.
...
- 아래의 작업을 거친 후 프로세스를 종료시킴
    1. atexit(3) 함수에 등록된 함수를 등록된 역순으로 호출한다.
    2. 열려 있는 모든 출력 스트림을 flush(버퍼를 비우는 것)한다.
    3. 열려 있는 스트림을 모두 닫는다.
    4. tmpfile(3) 함수로 작성된 모든 파일의 연결을 해제한다.
