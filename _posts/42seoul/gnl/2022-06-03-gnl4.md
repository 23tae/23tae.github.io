...
지역변수로 선언되었다면 문자열을 잃어버려 메모리 누수 발생한다.
...
- 동작방식
	...
	- 기존 backup에 읽어들인 buffer 이어 붙인다.(ft_strjoin 기존 함수와의 차이점: NULL이 들어왔을 때 처리)
...
BUFFER_SIZE 대신 OPEN_MAX를 넣어도 원하는 결과를 낼 수 있다.
