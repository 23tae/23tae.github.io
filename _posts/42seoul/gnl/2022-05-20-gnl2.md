...
- read 함수
    ...
    - 동작
        - `fd`가 가리키는 파일에서 `nbyte`만큼을 가져와서 `buf`가 가리키는 주소에 읽는다.
    - 리턴값
...
- gcc -D
    ...
    - `#define`을 프로그램 외부에서 정의하고 컴파일 시 반영할 수 있다.
...
- 디버거 관련
	- Vscode Debugger
	- [valgrind](https://github.com/LouisBrunner/valgrind-macos) (Mac OS에서는 Catalina까지만 지원된다고 한다.)
	- lldb
