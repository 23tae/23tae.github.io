...
- case에 break를 사용하지 않으면 다음과 같이 implicit-fallthrough 경고가 발생한다.
    ```shell
    Harl.cpp:44:24: error: this statement may fall through [-Werror=implicit-fallthrough=]
        44 |             this->debug();
           |             ~~~~~~~~~~~^~
    ```
    - 이를 방지하기 위해 사용하는 `[[fallthrough]]` 키워드는 C++17 이후부터 사용이 가능하다.
    - 대신 이전 버전에서는 주석으로 다음과 같이 fall through임을 명시하여 컴파일 경고를 피할 수 있다.
        ```cpp
        /* Falls through. */
        /* fall-thru */
        /* Else falls through. */
...
- [C++ int를 string으로 변경하는 방법](https://psychoria.tistory.com/708)
...
- [-Wimplicit-fallthrough in GCC 7 \| Red Hat Developer](https://developers.redhat.com/blog/2017/03/10/wimplicit-fallthrough-in-gcc-7)