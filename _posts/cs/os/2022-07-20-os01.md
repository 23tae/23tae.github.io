...
- 애플리케이션 프로그램의 기반을 제공한다.
- 사용자와 하드웨어의 매개체 역할을 한다.

## 부팅 과정

- 컴퓨터의 전원이 켜진다.
- CPU가 ROM의 부트스트랩 프로그램을 실행한다.
...
일반적으로 스토리지의 가격&접근속도와 용량은 반비례 관계이다.
...
OS 코드 중 상당분이 I/O를 관리하기 위해 사용된다.

DMA(Direct memory access) : 입출력장치와 메모리 사이 데이터 흐름에서 CPU를 배제하기 위해 사용하는 입출력 제어 방식 (예시. 유튜브로 영상을 시청하는 경우 네트워크에서 데이터를 받아 모니터에 바로 출력하면 돼서 CPU의 개입이 필요없다.)
...
- 여러 프로세서가 동일한 메모리에 접근시에는 1개만 접근 가능하고 나머지는 대기해야한다.
...
한번에 다수의 프로그램을 실행해 CPU의 활용률을 높인다.
...
- CPU 스케줄링
    ...
    - 다수의 프로세스가 한번에 실행되려면 시스템이 각각의 프로세스마다의 실행순서를 지정해야한다. 이럴 때 사용되는 것이 CPU 스케줄링.

## Operation mode

리소스 접근 권한에 따라 크게 **유저 모드**와 **커널 모드**로 나뉘어진다.

- 리소스 : 프로그램이 활용할 수 있는 데이터나 루틴

### 유저 모드

- 사용자가 접근할 수 있는 영역을 제한적으로 두고 프로그램의 리소스에 침범하지 못하게 한다.
- 일반적으로 사용자가 작업을 하는 영역

### 커널 모드

- 모든 리소스에 접근하여 명령을 할 수 있다.
...
가상화는 한 컴퓨터에서 다수의 실행환경을 추상화할 수 있게 한다.
