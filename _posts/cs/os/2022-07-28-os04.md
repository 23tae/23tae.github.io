...
이전까지는 프로세스가 하나의 스레드로 실행된다고 가정함  
하지만 실제로는 다중 스레드로 실행될 수 있음
...
## 멀티스레딩(multithreading)의 등장 배경

아래와 같은 클라이언트-서버 시스템(웹서버 등)을 가정하면,

기존의 싱글스레딩은 서버가 클라이언트로부터 request를 수신한 뒤에 해당 request를 처리하는 동안 기존의 서버로부터 수신하던 작업은 잠시 멈추게 됨.

하지만 멀티스레딩을 사용하게 되면 1과 3 사이에 2의 과정을 통해 새로 생성한 스레드에 request 처리 작업을 맡기고 기존의 작업을 계속해서 할 수 있음.
...
- **리소스의 공유(Resource Sharing)**
    - 프로세스의 리소스를 스레드가 공유해서 **스레드간 협업**이 가능
...
앞부분 멀티스레딩의 배경 파트에서는 요청이 들어오면 해당 요청을 처리할 스레드를 생성한다고 함. 하지만 스레드 생성은 프로세스 생성보다는 낫지만 여전히 잠재적인 문제가 있음.
