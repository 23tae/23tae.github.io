...
## 챕터 목표
- 프로세스의 구성 요소를 확인하고 운영 체제에서 이것들이 어떻게 나타나고 스케줄되는지를 설명한다.
- 운영 체제에서 프로세스가 생성되고 종료되는 방법에 대해 시스템 콜을 사용해 프로그램을 개발하는 등의 방식으로 설명한다.
- 프로세스 간 통신(Inter-process communication) 방법인 공유 메모리(shared memory)와 메시지 전달(message passing)을 설명하고 대조한다.
- 파이프와 POSIX 공유 메모리를 사용하여 프로세스 간 통신을 수행하는 프로그램을 설계한다.
- 소켓 및 원격 프로시저 호출을 통한 클라이언트 – 서버 통신에 대해 설명한다.
- Linux 운영 체제와 상호 작용하는 커널 모듈을 설계한다.

# Process Concept

## 프로세스란?

- **프로세스**는 실행되는 프로그램을 의미한다.
- 프로세스는 OS에서의 작업 단위
- 프로세스가 작업을 마치기 위해서는 다음과 같은 리소스를 필요로 한다.
    - CPU 타임
    - 메모리
    - 파일
    - I/O 디바이스
...
- Waiting : 어떤 이벤트의 발생을 기다리는 상태
    - 타임쉐어링으로 다른 프로세스를 잠시 실행하면 기존의 프로세스가 waiting 상태가 된다.
    - waiting queue에서 I/O 완료나 시그널의 수신 등을 기다린다.
- Ready : ready queue에서 프로세서에게 할당되기를 기다린다.
...
- Program counter : 마이크로프로세서 내부에 있는 레지스터 중의 하나로서, 다음에 실행될 명령어의 주소를 가지고 있어 실행할 기계어 코드의 위치를 지정한다.
- CPU resgisters
...
## 프로세스의 정의
### 기존 정의
- **싱글스레드로 실행**되는 프로그램을 말한다.
	- 스레드(thread) : 가벼운 프로세스를 의미한다.
- 한번에 한 개의 작업만 할 수 있다.

### 확장된 정의
- 한 개 이상의 스레드로 실행되는 프로그램
- 한번에 여러 개의 작업을 수행할 수 있다.
...
**목적** : 사용자에게 프로그램들이 동시에 실행된다고 느끼게 한다.
...
- 특정한 이벤트의 발생을 기다리는 프로세스는 **waiting queue**에 위치한다
    - **waiting queue**에서 기다리다가 **ready queue**로 이동한다.
- PCB에 연결리스트로 다음과 같이 구현되어있다.
...
- context
    - 프로세스가 사용되고 있는 상태. PCB에 저장되어 있다.
    - 보통 PCB 정보를 의미한다.
- interrupt가 발생하면
    - 시스템이 현재 실행되는 프로세스의 context를 저장하고
    - 나중에 해당 context를 복구해서 프로세스가 이어지게끔 한다.
- context switch의 과정은 아래와 같다.
...
하나의 프로세스는 다수의 새로운 프로세스를 생성할 수 있다.
...
- 실행의 2가지 경우의 수
    - parent process가 그것의 child process와 동시에(concurrently) 실행된다.
    - parent가 children이 종료될 때까지 기다린다.
- 주소 공간(address-space)의 2가지 경우의 수
    - child process가 parent의 복제품인 경우. 이 경우 같은 공간을 사용한다.
...
이렇게 생성된 child 프로세스는 parent 프로세스의 주소공간의 사본을 가진다.
...
child가 실행되는 동안 parent가 할 작업이 없으면 `wait()` 시스템 콜을 호출한 뒤, **ready queue**로 가서 child의 종료를 기다릴 수 있다. child가 종료되며 인터럽트를 주면 그때부터 다시 parent가 이어서 실행된다.
...
- 마지막 문장까지 실행을 완료한 경우
- `exit()` 시스템 콜을 호출한 경우 : OS에게 프로세스를 삭제할 것을 요청한다.

프로세스가 종료될 때 OS는 할당된 것들을 해제하고, 리소스를 되찾다.

- 메모리 할당, 파일 오픈, I/O 버퍼 등

### Zombie , Orphan

보통 parent는 child가 종료될 때까지 기다린다.
...
- 독립적으로 실행 (independent process)
    - 다른 프로세스들과 데이터를 공유하지 않다.
- 협력해서 실행 (cooperating process)
    - 다른 프로세스들과 데이터를 공유한다.
    - 다른 프로세스에게 영향을 주거나 받는다.

## IPC (Inter-Process Communication)

- Cooperating process들은 **IPC 메커니즘**을 필요로 한다.
    - 프로세스 간에 데이터를 주고받을 수 있다.

## IPC 모델

- (a) 메모리를 공유하는 방식 (Shared-Memory Systems)
    - 공유하는 메모리 영역을 통해 데이터를 주고받는다.
- (b) 메시지를 주고받는 방식 (Message-Passing Systems)
    - message queue에 메시지를 보내면 OS가 대신 전해준다.
...
- **기존 문제** : 생산자-소비자 문제 (producer-consumer problem)
    - 여러 개의 프로세스를 어떻게 동기화할 것인가에 관한 문제. 한정 버퍼 문제라고도 한다.
...
- **기존 문제**
    - shared memory를 관리하는 것은 온전히 개발자의 몫이다.
    - 생산자-소비자의 수가 늘어날 수록 개발자에게는 어려움이 존재.
- **해결 방법** : message-passing
- OS가 메시지를 전달할 수단(message-passing facility)를 제공해준다.
- message-passing facility의 동작
    - send (message)
    - receive (message)
- Communication Links
    - 두 프로세스간에 통신을 한다고 하면, 데이터를 주고 받는 데 사용되는 연결을 의미한다.
    - 형성되는 방식
...
- **direct / indirect** communication
    - direct
        - 통신을 하려는 프로세스는 수신자, 발송자로 명시된다.
        - send(P, message), receive(Q, message)의 형태로 메시지를 주고받는다. (P : 수신자, Q : 발송자)
        - 링크는 자동적으로 형성된다. (수신, 발신자가 정해져 있기 때문)
        - 링크는 두 개의 프로세스와만 관련된다.
        - 각 프로세스 쌍 별로 한 개의 링크만 존재한다.
    - indirect
        - mailbox (또는 port)를 통해 메시지를 주고받는다.
            - mailbox : 메시지가 각 프로세스에 의해 놓아지고 제거되는 추상적 공간
        - send(A, message), receive(A, message)의 형태로 메시지를 주고받는다. (A : mailbox)
        - 링크는 각 프로세스의 쌍이 공유하는 mailbox가 존재하는 경우에만 형성된다.
        - 링크는 두 개 이상의 프로세스와 관련될 수 있다.
        - 여러개의 링크가 존재할 수 있다. (각 링크는 하나의 mailbox를 가진다.)
        - OS의 역할
...
- **synchronous / asynchronous** communication (or **blocking / non-blocking**)
    ...
    - asynchronous communicatoin (비동기 통신)
        - non-blocking send : 발송자는 계속해서 메시지를 보낸다. (수신 여부와 상관없이)
        - non-blocking receive : 수신자는 계속해서 메시지를 받는다. (유효한 메시지 또는 null 메시지에 관계없이)
...
memory-mapped 파일을 이용해 메모리를 구성한다.
...
- 파이프는 두 프로세스가 통신할 수 있는 수도관 역할을 한다.
- pipe 구현의 네 가지 이슈
...
- pipe 종류
    - Ordinary pipes
        - 생성한 프로세스 외부에서 접속 불가
        - 주로, parent 프로세스가 child와 통신하기 위해 파이프를 생성한다.
        - 두 프로세스가 생산자-소비자의 방식으로 통신한다.
            - **생산자**는 pipe의 한 쪽 끝에다가 작성한다. (write end)
            - **소비자**는 pipe의 반대쪽 끝에서부터 읽는다. (read end)
