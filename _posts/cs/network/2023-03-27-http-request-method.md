...
- 서버 상에 있는 **데이터를 업데이트**하거나 교체하기 위해 사용되는 메소드
...
2. 일치하는 URL이 location 블록에 존재하는 경우
    1. 블록이 `proxy_pass` 지시문을 포함한 경우
        1. 지시문에 지정된 백엔드 서버로 요청을 포워딩하고 응답 생성을 기다린다.
    2. 블록이 `root` 지시문을 포함한 경우
        1. 요청받은 파일을 `root` 지시문에 지정된 디렉토리에서 찾는다.
            1. 파일을 찾은 경우
                1. 해당 내용을 응답 본문에 포함시킨다.
            2. 파일이 찾지 못찾은 경우
                1. 클라이언트에게 `404 Not Found` 응답을 반환한다.
    3. 블록이 `fastcgi_pass` 지시어를 포함한 경우
        1. Nginx는 지시문에 명시된 FastCGI 서버로 요청을 포워딩하고 FastCGI가 응답을 생성하기까지 기다린다.
    4. 블록이 `try_files` 지시문을 포함한 경우
        1. 각각의 지정된 매개변수를 차례로 찾아보고 첫 번째로 찾은 것을 반환한다.
3. 일치하는 URL이 `location` 블록에 없는 경우
    1. 클라이언트에게 404 에러 응답을 반환한다.
...
2. 블록이 `try_files` 지시문을 포함한 경우
    1. 요청받는 리소스가 서버에 존재하는지를 확인한다.
        1. 존재한다면 : `404 Not Found` 응답을 반환한다.
            - 에러를 반환하는 이유?
                - HTTP 명세에는 PUT 요청이 들어오면 파일을 새로 생성하거나, 존재하는 파일을 새 파일로 교체한다고 나와있다.
                - 하지만 존재하는 파일이 있는 경우 PUT 요청의 의도가 불명확하기 때문에 기본설정인 에러 반환을 따른다.
                - `if_modified_since`, `if_unmodified_since` 지시문을 수정하여 처리 방법을 변경할 수 있다.
        2. 없다면 : 아래 과정을 이어간다.
...
5. 요청 처리에 성공한 경우
    1. `201 Created` 응답을 반환한다.
6. 요청 처리에 실패한 경우
    1. 적절한 에러 응답을 반환한다. (ex. `400 Bad Request`, `500 Internal Server Error` 등)
...
5. 해당 서버나 애플리케이션이 응답을 반환하는 경우
    1. 해당 응답을 클라이언트에게 돌려준다.
6. 반환하지 않는 경우
    1. 적합한 에러페이지를 생성해서 클라이언트에게 돌려준다.
